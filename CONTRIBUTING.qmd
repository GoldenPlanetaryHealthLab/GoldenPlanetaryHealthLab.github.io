...
GitHub repository: [GoldenPlanetaryHealthLab.github.io](https://github.com/GoldenPlanetaryHealthLab/GoldenPlanetaryHealthLab.github.io)
...
## How the Website is Actually Built

[Quarto](https://quarto.org/) is a scientific and technical publishing system built
on top of Pandoc, and driven by Markdown. Quarto Markdown files
(.qmd) can contain prose, code, and visualizations, 
and are incrementally rendered into a static website by first
evaluating the YAML front matter, running code chunks, and then
rendering the markdown into HTML. All of this is wrapped up in
the command `quarto render`. 

To build the website locally, you must of course have Quarto 
installed on your machine/environment. Once you have Quarto and the repo,
run the command `quarto preview` in the terminal while in the root 
directory of the repository, and Quarto will run `quarto render` and 
start a local server where you can view the website in your browser.

On GitHub, we have set up a GitHub Action that automatically
runs `quarto render` and _deploys_ the website's HTML files 
to GitHub Pages whenever a commit is added to the `main` branch.
This is why we enforce that all contributions go through GitHub Issues 
and Pull Requests, even small ones: it ensures that there is a clear 
record of what changed and why, and allows us to review changes before 
they are merged and deployed to the live website.

So the next time you want to add new information to the site,
and feel like learning something about CI/CD, take a closer look
at the GitHub Action that runs on this repository in
`.github/workflows/publish.yml`, and the accompanying set up in
the Repo Settings to learn how this process works! You might even want
to set up a similar workflow [for your own project or personal website](https://quarto.org/docs/websites/).
